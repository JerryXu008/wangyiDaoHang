...
@synthesize mbLoading, mLocalPath, mImageView, mbActShow, mImageType, mDefaultPath, delegate, OnImageLoad, userinfo;
...
+ (NSString *)GetLocalPathOfUrl:(NSString *)path {
    ...
    NSString *name = [CSImageView MD5String:path];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        mImageType = TImageType_FullFillCS;
        mbLoading = NO;
...
- (CGRect)GetLocalFrame:(UIImage *)image {
    ...
    if (mImageType == TImageType_AutoSizeCS) {
    ...
    else if (mImageType == TImageType_LeftAlignCS) {
        CGSize imageSize = image.size;
        int iWidth = self.frame.size.width;
        int iHeight = self.frame.size.width*imageSize.height/imageSize.width;
        if (iHeight > self.frame.size.height) {
    ...
    else if (mImageType == TImageType_TopAlignCS) {
        CGSize imageSize = image.size;
        int iWidth = self.frame.size.width;
        int iHeight = self.frame.size.width*imageSize.height/imageSize.width;
        if (iHeight > self.frame.size.height) {
...
- (UIImage *)GetLocalImage:(UIImage *)image {
    if (mImageType == TImageType_CutFillCS) {
...
- (void)GetImageByStr:(NSString *)path {
    [self Cancel];
    self.mLocalPath = [CSImageView GetLocalPathOfUrl:path];
