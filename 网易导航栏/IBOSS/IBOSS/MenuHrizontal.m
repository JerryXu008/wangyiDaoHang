...
#define ArrowWidth 100.0f
#define ArrowHeight 2.0f
#define ArrowY  (self.frame.size.height-2)
@interface MenuHrizontal()
@property(strong,nonatomic) UIImageView *arrow;
-(void)createArrow:(NSArray *)aItemsArray;
...
- (id)initWithFrame:(CGRect)frame ButtonItems:(NSArray *)aItemsArray
{
    ...
    if (self) {
        ...
        if (mScrollView == nil) {
            ...
            mScrollView.backgroundColor=[UIColor colorWithRed:243/255.0 green:243/255.0 blue:243/255.0 alpha:1.0];
        }
...
-(void)createArrow:(NSArray *)aItemsArray
{
    ...
    _arrow.image=[UIImage imageNamed:@"xiaoxixuanzhong.png" ];
    int count=aItemsArray.count;
    float width=Screen_Width/count;
    float x=(width-ArrowWidth)/2;
    _arrow.frame=CGRectMake(x, ArrowY, ArrowWidth, ArrowHeight);
...
-(void)changeButtonStateAtIndex:(NSInteger)aIndex{
    ...
    [self changeButtonsToNormalState];//取消所有按钮状态
    vButton.selected = YES;
    //移动arrow
    int vbframeX=vButton.frame.origin.x;
    int count=mButtonArray.count;
    float width=Screen_Width/count;
    float x= vbframeX+(width-ArrowWidth)/2;
    _arrow.frame=CGRectMake(x, ArrowY, ArrowWidth, ArrowHeight);
    //滚动
